...
if os.name == "nt":
    ...
    subprocess.run(f"docker pull python:{PYTHON_VERSION}-windowsservercore", check=True)
    pandas_base_dir = os.path.join(os.path.dirname(__file__, '..'))
    subprocess.run(f'cd {pandas_base_dir} && docker run -v "`pwd`:`pwd`" -w `pwd` '
                   f'python:{PYTHON_VERSION}-windowsservercore ci/test_wheels_windows.bat', check=True)
else:
    import pandas as pd
