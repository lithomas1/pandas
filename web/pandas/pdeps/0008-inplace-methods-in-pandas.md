...
Those downsides are still relevant for the cases where we keep `inplace=True`. Therefore,
it is also an option to have the method return the original object that was operated
on inplace when `inplace=True`.
